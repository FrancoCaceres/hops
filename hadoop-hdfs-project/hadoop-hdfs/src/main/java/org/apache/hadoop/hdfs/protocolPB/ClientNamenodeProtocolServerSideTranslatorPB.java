...
public class ClientNamenodeProtocolServerSideTranslatorPB
  ...
  @Override
  public CompleteS3ResponseProto completeS3(RpcController controller,
                                        CompleteS3RequestProto req) throws ServiceException {
    try {
      boolean result = server.completeS3(req.getSrc(), req.getClientName(),
              req.getVersionId(), req.getSize(), req.getChecksum(),
              req.hasFileId() ? req.getFileId() : HdfsConstantsClient.GRANDFATHER_INODE_ID,
              req.hasData() ? req.getData().toByteArray() : null);
      return CompleteS3ResponseProto.newBuilder().setResult(result).build();
    } catch (IOException e) {
      throw new ServiceException(e);
    }
  }
