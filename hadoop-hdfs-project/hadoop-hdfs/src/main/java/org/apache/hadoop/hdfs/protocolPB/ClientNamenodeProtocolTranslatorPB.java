...
public class ClientNamenodeProtocolTranslatorPB
  ...
  @Override
  public boolean completeS3(String src, String clientName, String versionId, long size, long checksum, long fileId,
                            final byte[] data)
          throws AccessControlException, FileNotFoundException, SafeModeException,
          UnresolvedLinkException, IOException {
    CompleteS3RequestProto.Builder req =
            CompleteS3RequestProto.newBuilder().setSrc(src).setClientName(clientName).setVersionId(versionId)
                    .setSize(size).setChecksum(checksum).setFileId(fileId);
    if(data!=null){
      req.setData(ByteString.copyFrom(data));
    }

    try {
      return rpcProxy.completeS3(null, req.build()).getResult();
    } catch (ServiceException e) {
      throw ProtobufHelper.getRemoteException(e);
    }
  }
